...
- Задание считается актуальным, если его статус "Не выполнено", "Выполнить после" меньше текущего времени, оно отсутствует в массиве уже выполняющихся заданий (статус исполняемой).
...
- для linux /
Скачивает файл по ссылке в заданную папку на диске, если полученное значение платформы соответствует данной операционной системе.
...
- для win32 пусть начинается с C:\
- для linux /
Если полученный тип платформы соответствует данной операционной системе, то устанавливае для платформы linux права на файл 777 (rwxrwxrwx), запускает скрипт.
...
##### Описание
Если полученный тип платформы соответствует данной операционной системе, то выполняет команду в командной строке операционной системы.
...
##### Описание
Анализирует состояние задачи (выполняется ли она, статус выполнения, отсрочку выполнения), анализирует тип задачи и запускает соответствующую функцию.
...
##### Описание
Уничтожает задачи старше 10 дней, обрезает слишком длинные ответы задач. Исправляет рассинхронизацию массивов выполненных и не выполненных задач. Снимает статус исполняемой с задач, находящихся в данном статуса больше 12 часов (зависших).
